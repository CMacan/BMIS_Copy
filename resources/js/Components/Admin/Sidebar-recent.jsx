...
export function Sidebar({ isCollapsed }) {
  ...
  return (
    ...
    >
      <div
        className={`p-3 border-b border-gray-700 flex items-center ${
      ...
      <nav className="flex-1 overflow-y-auto space-y-2 scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-transparent">
        <SidebarSection title="GENERAL" isCollapsed={isCollapsed}>
          ...
          <SidebarLink href="/admin/applications" icon={FileText} hasSubmenu isCollapsed={isCollapsed}>
            Sectoral
          </SidebarLink>
  ...
  return (
    <li className="list-none">
      <Link
        ...
        } py-2 text-xs transition-colors hover:bg-gray-700 rounded-lg`}
      >
        <Icon className="w-5 h-5 text-gray-300" />
        {!isCollapsed && <span className="flex-1 ml-2">{children}</span>}
      </Link>
        ...
        <ul className="ml-8 border-l border-gray-600 pl-4 space-y-1">
          {submenuItems[submenuKey].map((item) => (
            <li key={item.href} className="list-none">
              <Link
                href={item.href}
                className="block px-3 py-1.5 text-xs text-gray-300 hover:bg-gray-700 rounded-md"
              >
                {item.name}
              </Link>
            </li>
