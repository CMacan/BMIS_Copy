...
export default function LGBTIntakeForm() {
  ...
  const [data, setData] = useState({
    ...
    education: "",
    contact: "",
    lgbt_gender_identity: "",
    lgbt_sexual_orient: "",
    form_id: form_id,
    formType: "LGBT INTAKE FORM",
  })
  ...
  // Form options
  const genderIdentityOptions = ["Gay", "Lesbian", "Male", "Female", "Transgender", "Non-Binary", "Other"]
  const sexualOrientationOptions = ["Heterosexual", "Homosexual", "Bisexual", "Asexual", "Other"]
  ...
  const civilStatusOptions = ["Single", "Married", "Widowed", "Separated", "Live-in"]
  ...
  const nextStep = () => {
    ...
    if (currentStep === 2) {
      if (!data.lgbt_gender_identity) {
        newErrors.lgbt_gender_identity = "Gender Identity is required."
      }
      if (!data.lgbt_sexual_orient) {
        newErrors.lgbt_sexual_orient = "Sexual Orientation is required."
      }
    }
  ...
  const stepLabels = ["Personal Information", "Additional Details", "Signature"]

  return (
    <UserLayout>
      <Head title="LGBT Intake Form" />
      <div className="px-4 md:px-8 py-6 w-full max-w-5xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl md:text-3xl font-bold text-black-800">LGBT Intake Form</h1>
        ...
        <form  className="space-y-8">
          ...
          {currentStep === 1 && (
            <PersonalInformationStep
              ...
              formType="LGBT FORM INTAKE FORM"
            />
          )}

          {/* Step 2: LGBT-Specific Information */}
          {currentStep === 2 && (
            <AdditionalDetailsStep
              ...
              description="Please provide details about your gender identity and sexual orientation."
              icon={<Heart className="w-5 h-5 text-white-600" />}
              fields={[
                { name: "lgbt_gender_identity", label: "Gender Identity", options: genderIdentityOptions },
                { name: "lgbt_sexual_orient", label: "Sexual Orientation", options: sexualOrientationOptions }
              ]}
